...
function load_keyboard_color() {
    ...
    keys = $$("#keyboard_practice .KeyboardKey text").forEach((v, i) => {
        ...
        max_color_range = 126; // from 0 to 256
...
function process_data(data, dataset) {
    ...
    data = data.map(item => {
        if (item == ' ')
            return '<span style="color:#222">␣&#8203;</span>';
    ...
    if ((evt.key == to_type.innerText[0]) || (evt.key == ' ' && to_type.innerText[0] == '␣')) {
        ...
        if (to_type.innerText.length == 0) {
            ...
            if (final_res > getValue(dataset + "_max")) {
                saveValue(dataset + "_max", final_res);
            }

            saveValue(dataset + "_avg", Math.round((parseFloat(~~getValue(dataset + "_avg")) * parseFloat(~~getValue(dataset + "_count")) + final_res) / (~~parseFloat(getValue(dataset + "_count")) + 1) * 10) / 10);
            saveValue(dataset + "_count", parseFloat(~~getValue(dataset + "_count")) + 1);

            saveValue(dataset + "_last_result", final_res);
            saveValue(dataset + "_last_mistake_count", mistake_count);
            saveValue(dataset + "_last_mistake_precentage", Math.round(mistake_count / num_chars * 100));
